...
allowed_branch_slugs = [b["branch_slug"] for b in branch_rows] if branch_rows else []

if (
    "selected_branch_slug" not in st.session_state
    or st.session_state["selected_branch_slug"] not in allowed_branch_slugs
):
    primary_branch = next((b for b in branch_rows if b["is_primary"]), None) if branch_rows else None
    st.session_state["selected_branch_slug"] = (
        primary_branch["branch_slug"] if primary_branch else (allowed_branch_slugs[0] if allowed_branch_slugs else None)
    )
...
with st.form("provision_library_form"):
    ...
    api_base_url = st.text_input(
        ...
        help="Use the base URL only. Do not include /upload.",
    )
